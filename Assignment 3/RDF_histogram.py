...
first_trajectory, last_trajectory = int(len(trajectories)*(1/6)),len(trajectories) # our simulation: (0.5/2), their with Na: (1/7), their without Na: (1/6)
...
for atoms in trajectories[first_trajectory:last_trajectory]:
    ...
    for O_atom_index in O_atoms_indexes:
        
        # Get distance (angstrom) between atoms using the minimum image convention (mic)
        distances = atoms.get_distances(O_atom_index, O_atoms_indexes, mic=True) 
